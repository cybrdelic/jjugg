...
// Added preference persistence + reset filters support

const logger = createLogger("useApplicationsLogic");
...
export interface ApplicationsState {
  ...
  sortConfig: {
    column: keyof Application | "company.name";
    direction: "asc" | "desc";
  };
...
export interface ApplicationsActions {
  ...
  setQuickFilters: (
    filters: Partial<ApplicationsState["quickFilters"]>
  ) => void;
...
export interface ApplicationsHookReturn
  extends ApplicationsState,
    ApplicationsActions {
  // Computed Values
...
const stagesOrder: ApplicationStage[] = [
  "applied",
  "screening",
  "interview",
  "offer",
...
export function useApplicationsLogic(): ApplicationsHookReturn {
  ...
  const { createApplication, updateApplication, deleteApplication } =
    appDataCtx;
  ...
  const applicationsLoadingMore = (appDataCtx as any)
    .applicationsLoadingMore as boolean | undefined;
  ...
  const updateState = useCallback(
    <K extends keyof ApplicationsState>(
      key: K,
  ...
  const addStatusUpdate = useCallback(
    (message: string, appId: string | null) => {
      ...
      setTimeout(() => {
        setState((prev) => ({
          ...prev,
          statusUpdates: prev.statusUpdates.filter(
            (update) => update.id !== newId
          ),
        }));
      }, 3000);
    },
  ...
  const PREF_KEY = "applications.prefs.v1";
    ...
    try {
      ...
      if (raw) {
        ...
        setState((prev) => ({
          ...prev,
          visibleColumns:
            parsed.visibleColumns && parsed.visibleColumns.length
              ? parsed.visibleColumns
              : prev.visibleColumns,
          quickFilters: parsed.quickFilters
            ? { ...prev.quickFilters, ...parsed.quickFilters }
            : prev.quickFilters,
          tableViewDensity: parsed.tableViewDensity || prev.tableViewDensity,
          isAutosizeEnabled: parsed.isAutosizeEnabled ?? prev.isAutosizeEnabled,
        }));
      }
    } catch {
      /* ignore */
  ...
  // Persist prefs when core pieces change
  useEffect(() => {
    if (!state.mounted) return; // avoid overwriting before hydration complete
    try {
      const toStore = JSON.stringify({
        visibleColumns: state.visibleColumns,
        quickFilters: state.quickFilters,
        tableViewDensity: state.tableViewDensity,
        isAutosizeEnabled: state.isAutosizeEnabled,
      });
      localStorage.setItem(PREF_KEY, toStore);
    } catch {
      /* ignore quota */
    }
  }, [
    ...
    state.quickFilters,
    state.tableViewDensity,
    state.isAutosizeEnabled,
    state.mounted,
  ]);
  ...
  const filteredApplications = useMemo(() => {
        ...
        filtered = filtered.filter((app: Application) => {
          ...
          switch (column) {
            ...
            case "position":
              ...
              break;
            case "dateApplied":
              value = app.dateApplied.toLocaleDateString();
              break;
            case "stage":
              value = app.stage.charAt(0).toUpperCase() + app.stage.slice(1);
              break;
            case "location":
              value = app.location;
              break;
    ...
    if (state.quickFilters.stage !== "all") {
      filtered = filtered.filter(
        (app) => app.stage === state.quickFilters.stage
      );
    }
    ...
    if (state.quickFilters.dateRange !== "all") {
      ...
      if (daysBack) {
        ...
        state.quickFilters.dateRange === "custom" &&
        state.quickFilters.customDateRange
      ) {
  ...
  useEffect(() => {
    ...
    logger.debug("debouncedSearchTerm:", debouncedSearchTerm);
    logger.debug("debouncedColumnFilters:", debouncedColumnFilters);
    logger.debug("quickFilters:", state.quickFilters);
  }, [
    applicationsData,
    filteredApplications,
    debouncedSearchTerm,
  ...
  // Proactive follow-up suggestion: once per 24h recommend follow-ups for stale "applied" applications
  useEffect(() => {
    if (!applicationsData || !applicationsData.length) return;
    try {
      const LS_KEY_FOLLOW = "applications.followup.suggest.v1";
      const lastRunRaw = localStorage.getItem(LS_KEY_FOLLOW);
      const now = Date.now();
      // Run at most once per 24h
      if (lastRunRaw && now - Number(lastRunRaw) < 24 * 60 * 60 * 1000) return;

      const stale = applicationsData.filter((app) => {
        if (app.stage !== "applied") return false;
        const days = Math.floor(
          (now - app.dateApplied.getTime()) / (1000 * 60 * 60 * 24)
        );
        return days >= 7; // threshold
      });

      if (stale.length) {
        // Escalate wording if many stale
        const older = stale.filter(
          (app) =>
            (now - app.dateApplied.getTime()) / (1000 * 60 * 60 * 24) >= 14
        ).length;
        const msg =
          older > 0
            ? `Follow-up needed: ${stale.length} application(s) idle ( ${older} 14+ days )`
            : `Consider follow-ups for ${stale.length} application(s) idle 7+ days`;
        addStatusUpdate(msg, null);
      }
      localStorage.setItem(LS_KEY_FOLLOW, now.toString());
    } catch {
      /* ignore */
    }
  }, [applicationsData, addStatusUpdate]);
  ...
  const applicationStats = useMemo(() => {
    ...
    return {
      ...
      appliedThisWeek: apps.filter(
        (app) => new Date(app.dateApplied) >= thisWeek
      ).length,
      appliedThisMonth: apps.filter(
        (app) => new Date(app.dateApplied) >= thisMonth
      ).length,
      ...
      interviews: apps
        ...
        .filter(
          (interview) => !interview.completed && new Date(interview.date) > now
        ).length,
      interviewsThisWeek: apps
        .flatMap((app) => app.interviews || [])
        .filter((interview) => {
          const interviewDate = new Date(interview.date);
          return (
            interviewDate >= thisWeek &&
            interviewDate <= new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000)
          );
        }).length,
      ...
      withSalary: apps.filter((app) => app.salary && app.salary.trim() !== "")
        .length,
      ...
      responseRate:
        apps.length > 0
          ? Math.round(
              (apps.filter((app) => app.stage !== "applied").length /
                apps.length) *
                100
            )
          : 0,
      ...
      successRate:
        apps.length > 0
          ? Math.round(
              (apps.filter((app) => app.stage === "offer").length /
                apps.length) *
                100
            )
          : 0,
    ...
    () =>
      state.selectedApplication
        ? applicationsData?.find(
            (app: Application) => app.id === state.selectedApplication
          ) || null
        : null,
  ...
  const selectedApplications = useMemo(
    () =>
      state.selectedRows.length > 0
        ? applicationsData?.filter((app: Application) =>
            state.selectedRows.includes(app.id)
          ) || []
        : [],
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      if (
        !confirm(
          `Are you sure you want to delete ${state.selectedRows.length} application(s)?`
        )
      ) {
        return;
      }
      ...
      await Promise.all(
        state.selectedRows.map((appId) => deleteApplication(appId))
      );
      addStatusUpdate(
        `Deleted ${state.selectedRows.length} applications`,
        null
      );
  ...
  const handleStageChange = useCallback(
    async (appId: string, newStage: ApplicationStage) => {
      try {
        await updateApplication(appId, { stage: newStage });
        addStatusUpdate(
          `Moved to ${newStage.charAt(0).toUpperCase() + newStage.slice(1)}`,
          appId
        );
      } catch (error) {
  ...
  const handleBulkStageChange = useCallback(
    async (appIds: string[], newStage: ApplicationStage) => {
      try {
        await Promise.all(
          appIds.map((appId) => updateApplication(appId, { stage: newStage }))
        );
        addStatusUpdate(
          `Moved ${appIds.length} applications to ${
  ...
  const handleBulkEdit = useCallback(
    (appIds: string[]) => {
      ...
      if (appIds.length === 1) {
        ...
        addStatusUpdate(
          `Bulk edit for ${appIds.length} applications (feature coming soon)`,
          null
        );
      }
    },
  ...
  const handleIncrementStage = useCallback(
    async (appId: string) => {
      const app = applicationsData?.find((a: Application) => a.id === appId);
      if (
        app &&
        stagesOrder.indexOf(app.stage as ApplicationStage) <
          stagesOrder.length - 1
      ) {
        const newStage =
          stagesOrder[stagesOrder.indexOf(app.stage as ApplicationStage) + 1];
  ...
  const handleDecrementStage = useCallback(
    async (appId: string) => {
      const app = applicationsData?.find((a: Application) => a.id === appId);
  ...
  const handleKanbanDrop = useCallback(
    async (draggedItemId: string, targetStage: ApplicationStage) => {
      try {
        const draggedApp = applicationsData?.find(
          (app: Application) => app.id === draggedItemId
        );
        ...
        if (draggedApp.stage !== targetStage) {
          ...
          addStatusUpdate(
            `Moved to ${
              targetStage.charAt(0).toUpperCase() + targetStage.slice(1)
            }`,
            draggedItemId
          );
        }
  ...
  const handleEditApplication = useCallback((appId: string) => {
    ...
    logger.debug("Edit application:", appId);
  }, []);
  ...
  const handleRowClick = useCallback(
    (appId: string, e: React.MouseEvent) => {
      ...
      if (
        target.closest(".stage-container") ||
  ...
  const handleStageClick = useCallback(
    (appId: string, e: React.MouseEvent) => {
      ...
      updateState(
        "activeStageDropdown",
        state.activeStageDropdown === appId ? null : appId
      );
    },
  ...
  const setViewMode = useCallback(
    (mode: "table" | "kanban") => updateState("viewMode", mode),
    [updateState]
  );
  const setSortConfig = useCallback(
    (config: ApplicationsState["sortConfig"]) =>
      updateState("sortConfig", config),
    [updateState]
  );
  const setVisibleColumns = useCallback(
    (columns: string[]) => updateState("visibleColumns", columns),
    [updateState]
  );
  const setSelectedRows = useCallback(
    (rows: string[]) => updateState("selectedRows", rows),
    [updateState]
  );
  const setIsDetailModalVisible = useCallback(
    (visible: boolean) => updateState("isDetailModalVisible", visible),
    [updateState]
  );
  const setSelectedApplication = useCallback(
    (id: string | null) => updateState("selectedApplication", id),
    [updateState]
  );
  const setIsColumnMenuOpen = useCallback(
    (open: boolean) => updateState("isColumnMenuOpen", open),
    [updateState]
  );
  const setContextMenu = useCallback(
    (menu: ApplicationsState["contextMenu"]) =>
      updateState("contextMenu", menu),
    [updateState]
  );
  const setActiveStageDropdown = useCallback(
    (id: string | null) => updateState("activeStageDropdown", id),
    [updateState]
  );
  const setInlineEditingId = useCallback(
    (id: string | null) => updateState("inlineEditingId", id),
    [updateState]
  );
  const setColumnFilters = useCallback(
    (filters: Record<string, string>) => updateState("columnFilters", filters),
    [updateState]
  );
  ...
  const setShowAdvancedFilters = useCallback(
    (show: boolean) => updateState("showAdvancedFilters", show),
    [updateState]
  );
  const setTableViewDensity = useCallback(
    (density: ApplicationsState["tableViewDensity"]) =>
      updateState("tableViewDensity", density),
    [updateState]
  );
  const setIsMobileView = useCallback(
    (mobile: boolean) => updateState("isMobileView", mobile),
    [updateState]
  );
  const setIsAutosizeEnabled = useCallback(
    (enabled: boolean) => updateState("isAutosizeEnabled", enabled),
    [updateState]
  );
  ...
  const handleResetFilters = useCallback(() => {
    ...
    addStatusUpdate("Filters reset", null);
  }, [addStatusUpdate]);
