...
import { useReminders, useApplications } from '../../hooks/useData';
import type { Reminder, ReminderPriority, Application } from '../../types';
...
export default function Reminders() {
  ...
  const handleSubmitReminder = (e: React.FormEvent) => {
    ...
    try {
      ...
      // Find related application if specified
      const relatedApplication = newReminder.relatedApplicationId
        ? applications.data.find(app => app.id === newReminder.relatedApplicationId)
        : undefined;

      // Create new reminder using the data service
      ...
      reminders.create(newReminderData);
  ...
  const filteredReminders = reminders.data.filter(reminder => {
  ...
  // Show loading state
  if (reminders.loading || applications.loading) {
    return (
  ...
  // Show error state
  if (reminders.error || applications.error) {
    return (
      <section className="reminders-section reveal-element">
        <CardHeader
          title="Reminders"
        ...
        <div className="error-state">
          <p>Failed to load reminders: {reminders.error || applications.error}</p>
          <button onClick={() => { reminders.refresh(); applications.refresh(); }}>
            Retry
          </button>
        </div>
      </section>
  ...
  return (
    <section className="reminders-section reveal-element">
      <CardHeader
        title="Reminders"
        subtitle={`Track tasks and deadlines (${reminders.data.filter(r => r.status === 'pending').length} pending)`}
        accentColor="var(--accent-orange)"
        variant="default"
      >
        <button
            ...
            <div className="form-group">
              ...
              <EnhancedDropdown
                ...
                onChange={(value) =>
                  setNewReminder({
                    ...newReminder,
                    relatedApplicationId: Array.isArray(value) ? value[0] ?? '' : value
                  })
                }
                size="medium"
                searchable
