...
import { storage, STORAGE_KEYS, isValidData, migrateData } from './storage';
import type {
    Application,
    Company,
    ApplicationStage,
...
class CompanyService extends BaseDataService<Company> {
    ...
    getByIndustry(industry: string): Company[] {
        return this.getAll().filter(company =>
...
class ActivityService extends BaseDataService<Activity> {
    ...
    addStageChangeActivity(application: Application, newStage: ApplicationStage): Activity {
        const stageLabels = {
            applied: 'Applied',
            screening: 'Screening',
            interview: 'Interview',
        ...
        return this.create({
            type: newStage === 'offer' ? 'offer' : newStage === 'rejected' ? 'rejected' : 'interview',
...
class ReminderService extends BaseDataService<Reminder> {
    ...
    getUpcoming(limit?: number): Reminder[] {
        const upcoming = this.getAll()
            .filter(reminder => reminder.dueDate > new Date() && reminder.status === 'pending')
    ...
    getOverdue(): Reminder[] {
        return this.getAll().filter(reminder =>
    ...
    getByApplication(applicationId: string): Reminder[] {
        return this.getAll().filter(reminder =>
            reminder.relatedApplication?.id === applicationId
        );
    }
